...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $senha_confirm = htmlspecialchars(trim($_POST['senha_confirm']));

    if ($senha !== $senha_confirm || $senha === '') {
        $_SESSION['error_message'] = 'Senha não coincide a confirmação ou vazia';
        header("Location: index.php");
        exit;
    }

    if (!isset($nome) || $nome === '') {
        $_SESSION['error_message'] = 'Nome vazio';
        header("Location: index.php");
        exit;
    }

    if (!isset($email) || $email === '') {
        $_SESSION['error_message'] = 'Email vazio';
        header("Location: index.php");
        exit;
    }
